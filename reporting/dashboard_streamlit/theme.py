...
def dashboard_css(dark: bool) -> str:
    if dark:
        colors = {
            "page": "#050505", "panel": "#0b0b0b", "panel2": "#131313",
            "sidebar": "#343434", "ink": "#f3f0e9", "muted": "#aaa69f",
            "line": "#76736d", "soft": "#242424", "accent": "#f24a0a",
            "accent2": "#ff8a00", "teal": "#0c7884", "shadow": "rgba(0,0,0,.34)",
        }
    else:
        colors = {
            "page": "#f7f7f5", "panel": "#ffffff", "panel2": "#fbfbfa",
            "sidebar": "#d7d7d7", "ink": "#202020", "muted": "#77736e",
            "line": "#aaa7a2", "soft": "#eeeeec", "accent": "#f04408",
            "accent2": "#f08300", "teal": "#135b86", "shadow": "rgba(43,38,31,.10)",
        }
    return f"""
    <style>
    ...
    [class*="st-key-intake_meta_row"] div[data-testid="stPopover"] button {{ border:0; background:transparent;
      box-shadow:none; color:var(--ts-ink); font-weight:700; font-size:.95rem; padding:.3rem .2rem;
      justify-content:flex-start; }}
    [class*="st-key-intake_meta_row"] div[data-testid="stSelectbox"] > div > div {{ border:0;
      background:transparent; box-shadow:none; }}
    [class*="st-key-intake_meta_row"] div[data-testid="stSelectbox"] div[data-baseweb="select"] > div {{
      border:0; background:transparent; color:var(--ts-ink); font-weight:700; font-size:.95rem; }}
    [class*="st-key-intake_meta_row"] svg {{ color:var(--ts-muted); }}
    ...
    div[data-testid="stFormSubmitButton"] button {{ min-height:56px; min-width:180px; border:0;
      border-radius:999px; background:#d69a98; color:#fff; font-family:"Pretendard Variable",Pretendard,sans-serif;
      font-size:1.05rem; font-weight:800; letter-spacing:.02em; box-shadow:0 10px 22px rgba(214,154,152,.45); }}
    div[data-testid="stFormSubmitButton"] button:hover {{ background:#c98583; color:#fff;
      box-shadow:0 12px 26px rgba(201,133,131,.55); }}
    ...
    .ts-wordmark {{ display:flex; align-items:flex-end; gap:.65rem; color:var(--ts-ink); }}
    .ts-wordmark-text {{ font-family:"Poppins","Pretendard Variable",Pretendard,sans-serif; font-size:2.15rem;
      line-height:.82; font-weight:800; letter-spacing:-.02em; }}
    .ts-mark {{ display:inline-block; width:46px; height:46px; transform:translateY(-2px); }}
    ...
    .ts-landing .ts-wordmark-text {{ font-size:5.35rem; line-height:.68; }}
    .ts-landing .ts-mark {{ width:108px; height:108px; }}
    .ts-question-title {{ margin:2.05rem 0 0; font-family:"Pretendard Variable",Pretendard,sans-serif;
      font-size:clamp(2rem,3.2vw,3.15rem); font-weight:800; letter-spacing:-.03em; color:var(--ts-ink); }}
    .ts-compose-meta {{ max-width:980px; margin:.85rem auto 0; padding:.72rem 1rem; border-radius:22px;
      ...
      font-size:.9rem; }}
    .ts-top-question {{ display:flex; align-items:center; gap:1rem; margin:0 0 .45rem; padding:.68rem 1.3rem;
    ...
    .ts-top-question .search {{ font-size:1.6rem; }}
    .ts-sk {{ margin-left:auto; color:#e6002d; font-size:1.75rem; font-weight:950; font-style:italic; }}
    ...
    .ts-card h3 {{ margin:0 0 .45rem; padding-bottom:.35rem; border-bottom:1px solid var(--ts-line);
      color:var(--ts-ink); font-size:1.05rem; }}
    .ts-card ul {{ margin:.4rem 0 0; padding-left:1.15rem; }} .ts-card li {{ margin:.55rem 0; line-height:1.4; }}
    .ts-card-inner h3 {{ margin:0 0 .45rem; padding-bottom:.35rem; border-bottom:1px solid var(--ts-line);
      color:var(--ts-ink); font-size:1.05rem; }}
    ...
    .ts-swot {{ display:grid; grid-template-columns:1fr 1fr; }}
    .ts-swot-cell {{ min-height:92px; padding:.5rem .7rem; }}
    .ts-swot-cell:nth-child(odd) {{ border-right:1px solid var(--ts-line); }}
    .ts-swot-cell:nth-child(-n+2) {{ border-bottom:1px solid var(--ts-line); }}
    .ts-swot-cell h4 {{ margin:0 0 .4rem; font-size:1rem; }}
    .ts-swot-cell h4::first-letter {{ font-size:1.55rem; color:var(--ts-teal); }}
    .ts-swot-cell:nth-child(even) h4::first-letter {{ color:var(--ts-accent); }}
    .ts-swot-cell p {{ margin:.22rem 0; font-size:.76rem; line-height:1.35; }}
