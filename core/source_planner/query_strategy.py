...
def build_source_search_terms(source: PlannedSource, question_terms: list[str]) -> list[str]:
    ...
    if source.role == "official" or not topics:
        return question

    normalized_question = [_normalize(term) for term in question]
    overlapping = [
        topic
        for topic in topics
        if any(
    ...
    remaining_topics = [topic for topic in topics if topic not in overlapping]
    return list(dict.fromkeys([*overlapping, *remaining_topics, *question]))
