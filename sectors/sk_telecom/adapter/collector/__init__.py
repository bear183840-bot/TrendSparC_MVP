"""collector for the sk_telecom sector adapter.
...
_STAGE = "sectors.sk_telecom.adapter.collector"
...
def collect(source_plan: SourcePlan) -> list[SourceDocument]:
    """SK텔레콤 소스 플랜에 따라 등록된 도메인별 키워드 검색을 실행하여 문서를 수집합니다."""
    if not source_plan.planned_sources:
        raise PipelineStageError(
            stage=_STAGE,
            reason="template_only: no sources registered for sk_telecom",
        )
