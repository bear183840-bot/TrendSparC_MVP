...
def _load_system_prompt() -> str:
    # The three completeness rules below are shared, sector-neutral
    # extraction requirements (same ones sk_broadband's analyzer sends) -
    # they apply to this sector's claim_type/metric_point/comparison_point
    # shapes just as much as to a flat-field schema, so they belong in every
    # analyzer's prompt regardless of which schema style it uses. Appended
    # here rather than duplicated into the sector prompt file so a future
    # rule change (see tests/test_prompt_invariants.py) only has one place
    # to update per constant, matching the shared-module pattern the rest
