"""collector for the sk_innovation sector adapter.
...
here fabricates content, and no reliability_tier is invented (only carried
through when the registry itself sets one).
"""
...
_STAGE = "sectors.sk_innovation.adapter.collector"
...
def collect(source_plan: SourcePlan) -> list[SourceDocument]:
    """SK이노베이션 소스 플랜에 따라 등록된 도메인별 키워드 검색을 실행하여 문서를 수집합니다."""
    if not source_plan.planned_sources:
        raise PipelineStageError(
            stage=_STAGE,
            reason="template_only: no sources registered for sk_innovation",
        )
