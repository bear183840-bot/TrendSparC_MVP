...
**아직 안 만든 변형** (자료가 오면 붙일 자리):
`CompetitorPanels`(경쟁사별 프로세스+중요도+도넛 묶음)는 claim을 경쟁사 단위로
묶는 필드가 없어서 미구현. `AiInsightButton`은 지금 상시 노출 캡션으로 대체돼
있고, 버튼+패널 인터랙션은 UI 작업으로 남아 있다.
